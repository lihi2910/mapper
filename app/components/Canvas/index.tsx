...
import translationsJson from "@/public/trsanslations.json";

type TypeUnitMap = Record<string, Record<string, number>>;

const translations: TypeUnitMap = translationsJson;
...
const Canvas: FC<CanvasProps> = (props) => {
    ...
    useEffect(() => {
        ...
        const sortedByX = dataAsArray
            .filter(([location, _value]) => translations[location])
            .sort(
                ([locationA, _A], [locationB, _B]) =>
                    translations[locationA].x - translations[locationB].x
            );
        const middle = Math.ceil(sortedByX.length / 2);
        const sortedLeft = sortedByX
            .splice(0, middle)
            .sort(
                ([locationA, _A], [locationB, _B]) =>
                    translations[locationA].y - translations[locationB].y
            );
        generateBoxes(sortedLeft as [string, TypeUnitMap][], true);
        const sortedRight = sortedByX
            .splice(-middle)
            .sort(
                ([locationA, _A], [locationB, _B]) =>
                    translations[locationA].y - translations[locationB].y
    ...
    const generateBoxes = (items: [string, TypeUnitMap][], isLeft: boolean) => {
        ...
        items.forEach(([location, value]) => {
            ...
            generateArrow(
                { x: x + sideDiff, y },
                { x: isLeft ? strokeX : boxX - 20, y: currentY - 20 + boxHeight / 2 }
            );
    ...
    const getItemWidth = (items: Record<string, number>, location: string) => {
        const longest = Object.entries(items).reduce((prev, [key, value]) => {
            return `${key}:  ${value}`.length > prev.length
                ? `${key}:  ${value}`
                : prev;
        }, "");
        ctx!.font = "15px Arial";
    ...
    const generateExplainBox = () => {
        ctx!.strokeStyle = "transparent";
        ctx!.fillStyle = "#f9665e";
        ctx!.beginPath();
        ctx!.roundRect(canvasWidth - 50, canvasHeight - 45, 22, 22, 5)
        ctx?.fill();
        ctx!.stroke();

        ctx!.fillStyle = "black";
        ctx!.fillText("מילואים", canvasWidth - 55, canvasHeight - 30);

        ctx!.fillStyle = "#99C3EC";
        ctx!.beginPath();
        ctx!.roundRect(canvasWidth - 50, canvasHeight - 75, 22, 22, 5)
        ctx?.fill();
        ctx!.stroke();

        ctx!.fillStyle = "black";
        ctx!.fillText("סדיר", canvasWidth - 55, canvasHeight - 60);
    }
